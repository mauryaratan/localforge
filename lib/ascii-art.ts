...
export type TextFont = "standard" | "banner" | "big" | "block" | "bubble" | "digital" | "lean" | "mini" | "script" | "shadow" | "slant" | "small";
...
export const convertImageToAscii = async (
  file: File,
  options: ConversionOptions
): Promise<{ ascii: string; html: string; dimensions: { width: number; height: number } }> => {
  ...
  const charAspectRatio = 0.5;
  const outputHeight = Math.round(options.width / aspectRatio * charAspectRatio);
 ...
 */
export const downloadAsciiArt = (ascii: string, filename = "ascii-art.txt"): void => {
