...
// biome-ignore-all lint/correctness/useImageSize: local blob previews render inside measured containers
...
const getPresetLevel = (quality: number): PresetLevel => {
  if (quality < 33) {
    return 0;
  }
  if (quality < 66) {
...
export default function ImageCompressorPage() {
  ...
  useEffect(() => {
    const updateSize = () => {
      if (!(selectedJob && imageAreaRef.current)) {
        return;
      }
  ...
  const buildCompressOptions = useCallback(
    (job: ImageJob) => ({
      ...
      preset: getPresetLevel(globalOptions.quality),
      subsampling420: true,
  ...
  const buildResizeOptions = useCallback(() => {
    if (!globalOptions.resizeEnabled) {
      return null;
    }
    return {
  ...
  const handleDownload = useCallback(() => {
    ...
    const name = selectedJob.name.replace(FILE_EXTENSION_REGEX, ext);
  ...
  return (
    <>
      {/* biome-ignore lint/a11y/noNoninteractiveElementInteractions: container accepts drag-and-drop files across the workspace */}
      {/* biome-ignore lint/a11y/noStaticElementInteractions: container accepts drag-and-drop files across the workspace */}
      <div
      ...
      >
        ...
        <div className="relative flex-1 overflow-auto" ref={imageAreaRef}>
          <div className="flex min-h-full min-w-full items-center justify-center p-4">
            {/* biome-ignore lint/a11y/noNoninteractiveElementInteractions: image compare surface supports pointer drag gestures */}
            {/* biome-ignore lint/a11y/noStaticElementInteractions: image compare surface supports pointer drag gestures */}
            <div
        ...
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-white/10 border-t px-4 py-2.5 text-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex overflow-hidden rounded border border-white/20">
              ...
              <button
                ...
                onClick={() =>
                  setGlobalOptions((o) => ({ ...o, format: "png" }))
                }
                type="button"
              >
                PNG
          ...
          {globalOptions.format === "png" && (
            <div className="flex items-center gap-1.5">
              <Switch
                aria-label="Toggle lossless PNG output"
          ...
          <div className="flex items-center gap-1.5">
            <Switch
              aria-label="Toggle image resizing"
            ...
            <>
              <div className="flex items-center gap-1.5">
                ...
                <Input
                  ...
                  min={1}
                  onChange={(e) =>
                    handleResizeWidthChange(Number.parseInt(e.target.value, 10))
                  }
                  type="number"
              ...
              <div className="flex items-center gap-1.5">
                <Switch
                  aria-label="Maintain aspect ratio while resizing"
